...
if (empty($mbFlag) || empty($num ) || (empty($start) && $start!=0)) {
    $temp = false;
}
 ...
 */
if ($temp) {
    $sql = "select * from db_member where mbFlag = '{$mbFlag}' order by mbId asc limit {$start},{$num}";
    $sql2="select count(*) from db_member where mbFlag = '{$mbFlag}' ";
